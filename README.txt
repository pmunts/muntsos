...
News
     ...
     * 16 March 2023 -- Cross-toolchain RPM packages and tarballs are now
       ...
       on Fedora 37, RHEL 9.1 and its derivatives CentOS Stream 9,
       AlmaLinux 9.1, and Rocky Linux 9.1. Available at:
       [3]http://repo.munts.com/muntsos/rpms and
     ...
     * 20 September 2023 -- After a long (years) hiatus, I have
       ...
       [5]Linaro ABE (Advanced Build Environment) using manifests derived
       from the latest [6]Arm GNU Toolchain release, Version 12.3.Rel1
       July 28, 2023. These GCC 12.3.1 toolchains are significantly
       fresher that what can currently be built using [7]Crosstool-NG.
       ...
       [8]http://repo.munts.com/muntsos/7 and stuff built with Linaro ABE
       on Debian 12 to [9]http://repo.munts.com/muntsos/8. I have modified
       ...
       Debian 12, you can continue to use the Crosstool-NG
       cross-toolchains or you can use the new Linaro ABE cross-toolchains
       installed by the new muntsos-dev-abe meta-package.

Quick Setup Instructions for the Impatient

   Instructions for installing the MuntsOS cross-toolchain development
   environment onto a host computer are found in [10]Application Note #1,
   [11]Application Note #2, and [12]Application Note #8. Or just download
   ...
   [16]Application Note #3 and [17]Application Note #15.
...
Documentation
   ...
   [18]http://git.munts.com/muntsos/doc
...
Embedded Linux Distribution in a Kernel
   ...
   more [19]device tree files (.dtb) that are specific to particular
   ...
   [20]http://repo.munts.com/muntsos/kernels
...
Extensions
   ...
   packages from the [21]Debian project will probably not work with
   ...
   The [22]GPIO Server extension package demonstrates how to build a
   ...
   [23]http://repo.munts.com/muntsos/extensions
...
Thin Servers
  ...
  Boot Files + Kernel Files + Extensions = Thin Server
   ...
   Note: BeagleBone boards require the [24]boot flag to be set on the
   ...
   bootable.
   MuntsOS Application Notes [25]3 and [26]15 contain more detailed
   instructions about how to install a MuntsOS Thin Server.

   Prebuilt MuntsOS Thin Servers are at available at:

   [27]http://repo.munts.com/muntsos/thinservers
...
Boards
  ...
  BeagleBone
   ...
   The [28]BeagleBone was one of the first low cost Linux microcomputers.
   It originally sold for USD $89 at its launch in October 2011.

   The BeagleBone has a Texas Instruments [29]Sitara AM3359 processor
   ...
   to the AM3359 console serial port. The BeagleBone also has two [30]PRU
   ...
   are are enabled on the two 46-pin [31]expansion headers:

     * I^2C bus controller device /dev/i2c-2
   ...
   processor are apparently still available from [32]Special Computing.

  BeagleBone Black

   The [33]BeagleBone Black is a cost reduced version of the BeagleBone.
   ...
   pins and peripheral ports available on its two 46-pin [34]expansion
   headers. Even after eMMC, I^2C, SPI, and UART pins have been allocated,
   there are 42 GPIO pins available.

   The BeagleBone Black has a Texas Instruments [35]Sitara AM3358
  ...
  BeagleBone Black Wireless
   ...
   The [36]BeagleBone Black Wireless is a variant of the BeagleBone Black
  ...
  BeagleBone Green
   ...
   The [37]BeagleBone Green is a cost reduced version of the BeagleBone
   Black, from Chinese manufacturer [38]Seeed Studio, that sells for about
   USD $44. Changes from the BeagleBone Black design are:
     ...
     * Added two [39]Grove System connectors, one carrying 3.3V [40]I^2C
       signals and one carrying 3.3V logic level [41]serial port signals.
   ...
   but provides many more GPIO pins on its two 46-pin [42]expansion
  ...
  BeagleBone Green Wireless
   ...
   The [43]BeagleBone Green Wireless is a variant of the BeagleBone Green
   that has replaced the wired Ethernet interface with a built-in Wifi
   ...
   [44]BeagleBone Click Shield, which has some advantages over the newer
   [45]mikroBus Cape.

  PocketBeagle

   The [46]PocketBeagle is a cost and size reduced version of the
   ...
   following devices on its two 36-pin [47]expansion headers:

     * USB host port
     * I^2C bus controller device /dev/i2c-1
     * I^2C bus controller device /dev/i2c-2
     * PWM output 0:0
     * PWM output 2:0
   ...
   rows match the [48]MikroElektronika mikroBUS specification. If female
   sockets are installed on the top of the PocketBeagle, two [49]Click
  ...
  Raspberry Pi
   ...
   The [50]Raspberry Pi is a family of low cost Linux microcomputers
   ...
   With the advent of the [51]Raspberry Pi Zero 2, all Raspberry Pi 1
   models are now obsolete. MuntsOS Embedded Linux will continue to
   support all Raspberry Pi 1 models.

    Raspberry Pi 2 and 3

   The [52]Rasbperry Pi 2 Model B has a 900 MHz BCM2709 ARMv7 Cortex-A7
   ...
   The [53]Rasbperry Pi 3 Model B has a 1200 MHz BCM2710 ARMv8 Cortex-A53
   quad-core CPU and has 1 GB of RAM along with on-board Bluetooth and
   WiFi radios.

   The [54]Raspberry Pi 3 Model A+ has the same form factor as the
   ...
   The [55]Raspberry Pi 3 Model B+ has a 1400 MHz BCM2710 ARMv8 Cortex-A53
   quad-core CPU and has improved power management and networking
   components.

   The [56]Raspberry Pi Zero 2 has the same form factor as the Raspberry
   ...
   The [57]Raspberry Pi 4 Model B has a 1500 MHz BCM2711 ARMv8 Cortex-A72
   ...
   MuntsOS also provides Raspberry Pi kernels with dedicated [58]USB
...
Cross-Toolchains
   ...
   [59]Crosstool-NG) for each MuntsOS platform family. Each
   ...
   I also build [60]Free Pascal cross-compilers. These rely on the
   ...
   Cross-toolchain packages built for [61]Debian Linux are available at:
   [62]http://repo.munts.com/debian11

   RPM packages containing the exact same binaries and known to work on
   Fedora 37 are available at:
   [63]http://repo.munts.com/muntsos/rpms

   Tarballs containing the exact same binaries are available at:
   [64]http://repo.munts.com/muntsos/tarballs

Git Repository

   The source code for MuntsOS is available at:

   [65]https://github.com/pmunts/muntsos
...
File Repository
   ...
   [66]http://repo.munts.com/muntsos

[67]Make With Ada Projects

     * 2017 [68]Ada Embedded Linux Framework
     * 2019 [69]Modbus RTU Framework for Ada (Prize Winner!)
   _______________________________________________________________________

   Questions or comments to Philip Munts [70]phil@munts.net
...
References
   ...
   4. http://repo.munts.com/muntsos/tarballs
   5. https://wiki-archive.linaro.org/ABE
   6. https://developer.arm.com/downloads/-/arm-gnu-toolchain-downloads
   7. https://crosstool-ng.github.io/
   8. http://repo.munts.com/muntsos/7
   9. http://repo.munts.com/muntsos/8
  10. http://git.munts.com/muntsos/doc/AppNote1-Setup-Debian.pdf
  11. http://git.munts.com/muntsos/doc/AppNote2-Setup-Tarballs.pdf
  12. http://git.munts.com/muntsos/doc/AppNote8-Setup-RPM.pdf
  13. http://git.munts.com/muntsos/scripts/setup-debian
  14. http://git.munts.com/muntsos/scripts/setup-fedora
  15. http://git.munts.com/muntsos/scripts/setup-rhel
  ...
  18. http://git.munts.com/muntsos/doc
  19. http://elinux.org/Device_Tree_Reference
  20. http://repo.munts.com/muntsos/kernels
  21. http://www.debian.org/
  22. http://git.munts.com/muntsos/extensions/GPIO
  23. http://repo.munts.com/muntsos/extensions
  24. https://en.wikipedia.org/wiki/Boot_flag
  25. http://git.munts.com/muntsos/doc/AppNote3-Installation-from-Linux.pdf
  26. http://git.munts.com/muntsos/doc/AppNote15-Installation-from-Windows.pdf
  27. http://repo.munts.com/muntsos/thinservers
  28. http://beagleboard.org/bone-original
  29. http://www.ti.com/product/AM3359
  30. http://beagleboard.org/pru
  31. http://git.munts.com/muntsos/doc/BeagleBonePinout.pdf
  32. https://specialcomp.com/beagleboard/bone.htm
  33. http://beagleboard.org/black
  34. http://git.munts.com/muntsos/doc/BeagleBonePinout.pdf
  35. http://www.ti.com/product/AM3358
  36. https://beagleboard.org/black-wireless
  37. https://beagleboard.org/green
  38. https://www.seeedstudio.com/
  39. http://wiki.seeed.cc/Grove_System
  40. https://en.wikipedia.org/wiki/I2C
  41. https://en.wikipedia.org/wiki/Serial_port
  42. http://git.munts.com/muntsos/doc/BeagleBonePinout.pdf
  43. https://beagleboard.org/green-wireless
  44. https://www.mikroe.com/beaglebone
  45. https://www.mikroe.com/beaglebone-mikrobus-cape
  46. https://beagleboard.org/pocket
  47. http://git.munts.com/muntsos/doc/PocketBeaglePinout.pdf
  48. https://www.mikroe.com/mikrobus
  49. https://shop.mikroe.com/click
  50. http://www.raspberrypi.com/
  51. https://www.raspberrypi.com/products/raspberry-pi-zero-2-w
  52. https://www.raspberrypi.com/products/raspberry-pi-2-model-b
  53. https://www.raspberrypi.com/products/raspberry-pi-3-model-b
  54. https://www.raspberrypi.com/products/raspberry-pi-3-model-a-plus
  55. https://www.raspberrypi.com/products/raspberry-pi-3-model-b-plus
  56. https://www.raspberrypi.com/products/raspberry-pi-zero-2-w
  57. https://www.raspberrypi.com/products/raspberry-pi-4-model-b
  58. http://www.linux-usb.org/gadget
  59. https://crosstool-ng.github.io/
  60. https://www.freepascal.org/
  ...
  65. https://github.com/pmunts/muntsos
  66. http://repo.munts.com/muntsos
  67. https://www.makewithada.org/
  68. https://www.makewithada.org/entry/ada_linux_sensor_framework
  69. https://www.hackster.io/philip-munts/modbus-rtu-framework-for-ada-f33cc6
  70. mailto:phil@munts.net
